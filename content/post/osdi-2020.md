...
*Ansor* 是一个针对深度学习网络的张量计算优化和编译框架。类似 [Apache TVM](https://tvm.apache.org/)[OSDI'18, NIPS’18]，Ansor 可以用来优化深度学习网络在不同硬件架构（Intel、ARM、NVIDIA GPU）上的推理速度。Ansor 主要解决了两个问题：(1) 如何自动化的构造一个足够大的搜索空间（TVM 和 [Halide](https://halide-lang.org/)[PLDI'13, SIGGRAPH’12, SIGGRAPH’16, SIGGRAPH’18, SIGGRAPH'19] 的搜索空间不够大，同时 TVM 需要不少人工的工作）？Ansor 使用了一个层次化的搜索空间解决这个问题；(2) 如何有效的进行搜索？Ansor 先对完整的程序进行采样然后再进行调整。
...
这篇论文的第一作者[郑怜悯](http://lmzheng.net/)也是 TVM 的贡献者之一，[陈天奇](https://tqchen.com/)大牛的学弟。
